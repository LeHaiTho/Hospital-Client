...
const data = [
  ...
  {
    key: "3",
    fullname: "Lâm Minh Bau",

    schedule: "5 lịch hẹn",
    status: "Đang làm việc",
    email: "email@gmail.com",
    phone: "101010101001",
    specialties: "Thần kinh, Tai mũi họng",
  },
  {
    key: "4",
    fullname: "John Brown",
...
const DoctorList = () => {
  ...
  const handleFinish = async (values) => {
    try {
      const response = await axiosConfig.post("/doctors/create-doctor", values);
      await getDoctorList();
      form.resetFields();
      setVisible(false);
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "phone",
      key: "phone",
      render: (phone) => <p style={{ color: "gray" }}>+84 {phone}</p>,
    },
    {
      title: "Trạng thái",
      dataIndex: "status",
    ...
    >
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleFinish}>
          ...
          >
            <Input.TextArea placeholder="Nhập mô tả bác sĩ" rows={4} />
          </Form.Item>
          <Form.Item
            name="specialty"
